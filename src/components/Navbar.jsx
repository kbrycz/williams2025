...
function Navbar({ variant = 'transparent' }) {
  ...
  const navItems = [
    { name: 'Home', to: '/' },
    { name: 'About', to: '/about' },
    { name: `${siteData.candidate.firstName}'s Plan`, to: '/plan' },
  ...
  const mobileMenuBtnClass = isWhite
    ? 'xl:hidden p-2 rounded-lg hover:bg-neutral-100 transition-colors duration-300 -mt-10'
    : 'xl:hidden p-2 rounded-lg text-white hover:bg-white/10 transition-colors duration-300 -mt-10'

  const mobileMenuBtnColor = isWhite ? colors.primary[500] : 'currentColor'

  return (
    <nav className={`${navBg} z-50 transition-colors duration-300`}>
      <div className="w-full px-6 sm:px-8 xl:px-14 py-0">
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Navigation */}
          <div className="hidden xl:flex items-center gap-3 xl:gap-4 -mt-10">
          ...
          <div className="hidden xl:flex items-center gap-4 -mt-10">
            ...
            <div className={`flex items-center gap-2 transition-opacity duration-300 ${isWhite ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <a href={siteData.contact.social.facebook} className={socialBtnClass} aria-label="Facebook">
                <span className="font-bold text-xs">f</span>
              </a>
              <a href={siteData.contact.social.instagram} className={socialBtnClass} aria-label="Instagram">
              ...
              <a href={siteData.contact.social.twitter} className={socialBtnClass} aria-label="X (Twitter)">
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <AnimateIn delay={0.5}>
              <a
                ...
                className="bg-secondary-500 hover:bg-secondary-400 text-white font-bold px-7 py-2 rounded-md transition-all duration-200 uppercase tracking-widest text-sm hover:scale-105 hover:shadow-lg active:scale-95 inline-block"
              >
                Donate
              </a>
            </AnimateIn>
      ...
      {mobileMenuOpen && (
        <div
          className="xl:hidden fixed inset-0 z-[60] pb-8"
          style={{ backgroundColor: colors.primary[900] }}
        >
          {/* Duplicate header row */}
          <div className="flex items-center justify-between px-6 sm:px-8 py-4">
            <Link to="/" className="flex-shrink-0 block" onClick={() => setMobileMenuOpen(false)}>
              <img
                src={logoImage}
                alt={siteData.campaign.name}
                className="h-36 sm:h-40 w-auto ml-4"
              />
            </Link>
            <div className="flex items-center gap-3">
              <a
                href={siteData.campaign.donateUrl}
                className="bg-secondary-500 hover:bg-secondary-400 text-white font-bold px-5 py-2 rounded-md transition-colors duration-200 shadow-lg text-sm tracking-wider uppercase"
              >
                Donate
              </a>
              <button
          ...
          <div className="flex flex-col px-6 pt-2">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.to}
                className="text-neutral-200 text-lg font-bold tracking-[0.2em] uppercase py-4 border-b border-white/15 hover:text-secondary-400 transition-colors block"
                onClick={() => setMobileMenuOpen(false)}
              >
                {item.name}
              </Link>
